...
import { sendCareerApplication } from '@/components/api/careerapi';
...
const Careers = () => {
  ...
  const [securityCode, setSecurityCode] = useState(SECURITY_CODE);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const careerData = {
        ...
        phone: form.phone,
        email: form.email,
        applying_position: form.applyingFor,
        address: form.address,
        qualification: form.qualification,
        skills: form.skills,
        total_experience: `${form.totalExpYrs} years ${form.totalExpMth} months`,
        relevant_experience: form.relevantExp,
        current_ctc: `${form.currentCTCLakh}.${form.currentCTCK} LPA`,
        expected_ctc: `${form.expectedCTCLakh}.${form.expectedCTCK} LPA`,
        is_negotiable: form.isNegotiable,
        notice_period: form.noticePeriod,
        reason_for_change: form.reasonForChange,
        current_location: form.currentLocation,
        willing_to_relocate: form.willingToRelocate,
        preferred_contact_time: form.preferredContact,
        security_code: form.securityCode
      };

      await sendCareerApplication(careerData);
          ...
          {showModal && (
            <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
              <div className="bg-white rounded-xl shadow-lg p-8 max-w-sm w-full text-center">
                ...
                <p className="mb-6 text-gray-700">Your application has been submitted successfully.</p>
                <button
                  className="btn-modern bg-cloud-blue hover:bg-blue-600 w-full"
          ...
          <form onSubmit={handleSubmit} onReset={handleReset} className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                {validationErrors.phone && (
                  <div className="text-pink-400 text-sm mt-1">{validationErrors.phone}</div>
                )}
              </div>
            ...
            <div className="space-y-6">
              ...
              {validationErrors.email && (
                <div className="text-pink-400 text-sm mt-1">{validationErrors.email}</div>
              )}
